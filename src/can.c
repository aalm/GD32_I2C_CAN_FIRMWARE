...
*/
void
can_param_config(uint32_t can_periph, uint8_t *conf_str)
{
	uint32_t __can20baud = char2long(&conf_str[0]);
	...
	for (int i = 0; i < 14; i++) {
		if (conf_str[8 + i * 10]) {
			uint32_t __mask = char2long(&conf_str[i * 10 + 10]);
			uint32_t __filt = char2long(&conf_str[i * 10 + 14]);
			int __ext = conf_str[i * 10 + 9] ? CAN_EXTENDED_FIFO0 : CAN_STANDARD_FIFO0;

			if (CAN0 == can_periph) {
				can_filter_mask_mode_init(__filt, __mask, __ext, i);
#if DEBUG
				printf("CAN0 filter %d initialized to %ld, mask set to %ld\n", i, __filt, __mask);
#endif
			} else {
				can_filter_mask_mode_init(__filt, __mask, __ext, i + 14);
#if DEBUG
				printf("CAN1 filter %d initialized to %ld, mask set to %ld\n", i, __filt, __mask);
#endif
			}
		}
#if DEBUG
		else {
			printf("%s filter %d disabled\n", CAN0 == can_periph ? "CAN0" : "CAN1", i);
		}
#endif
	}
